...
int main()
{
    ...
    // first we test puts
    int data = s;
    bsp_put((s + 1) % p, &data, &a, 0, sizeof(int));
    bsp_put((s + 2) % p, &data, &b, 0, sizeof(int));
    for (int t = 0; t < p; ++t) {
        bsp_put(t, &data, &c, sizeof(int) * s, sizeof(int));
    }
    bsp_sync();
    ...
    bsp_get((s + 1) % p, &a, 0, &core_num_next, sizeof(int));
    bsp_get((s + 2) % p, &b, 0, &core_num_next_next, sizeof(int));
    bsp_get((s + 3) % p, &c, 4 * sizeof(int), &data, sizeof(int));
    bsp_sync();
