...
There are already some popular parallel-programming paradigms available for this architecture. In particular, the architecture is programmable using OpenCL. However, the only optimized way to program this architecture is by using the supplied software development kit, the Epiphany SDK (ESDK), which in our opinion lacks documentation especially for people new to the EA. In the report we describe a new library for the EA, which is built on top of the ESDK called Epiphany-BSP. 

The Bulk Synchronous Parallel (BSP) computing model is a model for designing parallel algorithms. It provides a description of how parallel computation should be carried out. Programs written with this model consist of a number of supersteps, which in turn consist of a local computations and non-blocking communication. A (barrier) synchronisation at the end of such a step is used to guarantee occurance of all communications within a step. Later we will describe the BSP model in some more detail. First we introduce th Epiphany architecture and present some technical details on the intented target hardware.
